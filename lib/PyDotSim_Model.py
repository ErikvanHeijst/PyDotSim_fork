...
QDshapeStr = ["Ellpisoid", "Disk", "Cone", "Cone-shell"]
            ...
            for ix in range(N):
                for iy in range(N):
                    for iz in range(N):
                        ...
                        d = np.sqrt(
                            pow((x - xPC), 2) + pow((y - yPC), 2) + pow((z - zPC), 2)
                        )
                        if d == 0:
                            d = model.cellSize / 4
                        Ve[ix, iy, iz] = Ve[ix, iy, iz] + q / (
                            4 * cnst.epsilon_GaAs * math.pi * d
                        )
                        Vh[ix, iy, iz] = Vh[ix, iy, iz] - q / (
                            4 * cnst.epsilon_GaAs * math.pi * d
                        )
        ...
        for ix in range(N):
            for iy in range(N):
                for iz in range(N):
                    ...
                    if rxy2 > 0:
                        Ve[ix, iy, iz] = (
                            Ve[ix, iy, iz] + c0_e / rxy2 + c1_e + c2_e * rxy2
                        )
                        Vh[ix, iy, iz] = (
                            Vh[ix, iy, iz] + c0_h / rxy2 + c1_h + c2_h * rxy2
                        )
...
def ShowPotential(figModel, canvasModel):
    ...
    xy_plane = figModel.add_subplot(131)
    xy_im = xy_plane.imshow(Ve[:, :, iz], interpolation="none", cmap=colMap)
    figModel.colorbar(xy_im)
    xy_plane.set_title("XY plane")

    xz_plane = figModel.add_subplot(132)
    xz_im = xz_plane.imshow(Ve[:, iy, :], interpolation="none", cmap=colMap)
    figModel.colorbar(xz_im)
    xz_plane.set_title("XZ plane")

    yz_plane = figModel.add_subplot(133)
    yz_im = yz_plane.imshow(Ve[ix, :, :], interpolation="none", cmap=colMap)
    figModel.colorbar(yz_im)
    yz_plane.set_title("YZ plane")

    figModel.tight_layout()
    canvasModel.draw()
