...
def addQDShape(ax):
    ...
    if QDshape == 1:
        rxQD = 1e9 * model.QDpara[1]
        ryQD = 1e9 * model.QDpara[2]
        rzQD = 1e9 * model.QDpara[3]
        pass
    # Disk, r1: r, r2: h
    if QDshape == 2:
        rQD = 1e9 * model.QDpara[1]
        hQD = 1e9 * model.QDpara[2]
        pass
    # Cone, r1: r, r2: h
    if QDshape == 3:
        rQD = 1e9 * model.QDpara[1]
    ...
    if QDshape == 4:
        rQD = model.QDpara[1] / model.cellSize
        hHole = model.QDpara[2] / model.cellSize
        hQD = model.QDpara[3] / model.cellSize
        ix0 = (model.cellNum1D - 1) / 2
        iz0 = ix0 - hHole / 2
        line = lines.Line2D([iz0, iz0 + hHole], [ix0, ix0 + rQD], lw=1, color="red")
        ax.add_line(line)
        line = lines.Line2D(
            [iz0 + hQD, iz0 + hHole], [ix0, ix0 + rQD], lw=1, color="red"
        )
        ax.add_line(line)
        line = lines.Line2D(
            [iz0 + hQD, iz0 + hHole], [ix0, ix0 - rQD], lw=1, color="red"
...
def showPSI(figResults, canvasResults):
    ...
    psie_xy = figResults.add_subplot(231)
    psie_xy_im = psie_xy.imshow(psie[0][:, :, i], interpolation=interpol, cmap=colMap)
    figResults.colorbar(psie_xy_im)
    psie_xy.set_title("WFe | XY")

    psie_xz = figResults.add_subplot(232)
    psie_xz_im = psie_xz.imshow(psie[0][:, i, :], interpolation=interpol, cmap=colMap)
    figResults.colorbar(psie_xz_im)
    psie_xz.set_title("WFe | XZ")

    psie_yz = figResults.add_subplot(233)
    psie_yz_im = psie_yz.imshow(psie[0][i, :, :], interpolation=interpol, cmap=colMap)
    figResults.colorbar(psie_yz_im)
    psie_yz.set_title("WFe | YZ")

    psih_xy = figResults.add_subplot(234)
    psih_xy_im = psih_xy.imshow(psih[0][:, :, i], interpolation=interpol, cmap=colMap)
    figResults.colorbar(psih_xy_im)
    psih_xy.set_title("WFh | XY")

    psih_xz = figResults.add_subplot(235)
    psih_xz_im = psih_xz.imshow(psih[0][:, i, :], interpolation=interpol, cmap=colMap)
    figResults.colorbar(psih_xz_im)
    psih_xz.set_title("WFh | XZ")

    psih_yz = figResults.add_subplot(236)
    psih_yz_im = psih_yz.imshow(psih[0][i, :, :], interpolation=interpol, cmap=colMap)
    figResults.colorbar(psih_yz_im)
    psih_yz.set_title("WFh | YZ")

    # figResults.add_subplot(231).imshow(
    #     psie[0][i, :, :], interpolation=interpol, cmap=colMap
    # )
    # figResults.add_subplot(232).imshow(
    #     psie[0][:, i, :], interpolation=interpol, cmap=colMap
    # )
    # figResults.add_subplot(233).imshow(
    #     psie[0][:, :, i], interpolation=interpol, cmap=colMap
    # )
    # figResults.add_subplot(234).imshow(
    #     psih[0][i, :, :], interpolation=interpol, cmap=colMap
    # )
    # figResults.add_subplot(235).imshow(
    #     psih[0][:, i, :], interpolation=interpol, cmap=colMap
    # )
    # figResults.add_subplot(236).imshow(
    #     psih[0][:, :, i], interpolation=interpol, cmap=colMap
    # )
    # add QD shape
