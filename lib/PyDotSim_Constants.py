...
indium_gallium_arsenide_qd = Alloy(
    host=indium_arsenide, guest=gallium_arsenide, x=0.6, C=0.477, name="InGaAs"
)
...
# Vacuum permittivity [F/m] = [A*s/(V*m)]
me0 = 9.10938188e-31
# Electron mass [kg]
c = 299792458
...
epsilon_GaAs = 13.1 * epsilon0
# GaAs permittivity
m_Ex = 1 / (1 / me_GaAs + 1 / mhh_GaAs)
# Exciton effective mass
r_Ex_GaAs = 4 * math.pi * epsilon_GaAs * hbar**2 / (m_Ex * e**2)
# GaAs exciton Bohr radius in [m]
E_Ry_GaAs = m_Ex * e**4 / (2 * (2 * epsilon_GaAs * h) ** 2)
# Rydberg constant in GaAs bulk in [J]
E_p = 28.8
# Kane energy in eV
n = 3.6
# refractive index
...
def GaAs_bandgap(T):  # Varshni
    ...
    # gallium_arsenide.calculate_derived_properties()
    EGaAs = indium_gallium_arsenide_qd._bandgap()
    return EGaAs
...
def QDlifetime(overlapp, E_PL):
    tau = (
        ...
        * epsilon0
        * me0
        / (math.pi * n * e**2 * E_p / JeV * E_PL / JeV * overlapp)
    )
    return tau
